...
export default function QuizPage() {
  ...
  const handleSubmit = () => {
    ...
    const safeTotal = questions.length || 0;
    const safeCorrect = Number.isFinite(correctCount) ? correctCount : 0;
    const safeScore = safeTotal > 0 ? Math.round((safeCorrect / safeTotal) * 100) : 0;

    const result = {
      setId: Number.isNaN(parsedSetId) ? 0 : parsedSetId,
      totalQuestions: safeTotal,
      correctAnswers: safeCorrect,
      wrongAnswers: Math.max(0, safeTotal - safeCorrect),
      score: safeScore,
    ...
    try {
      ...
      // Store current result separately with a unique ID
      const resultId = `result_${Date.now()}`;
      localStorage.setItem(resultId, JSON.stringify(result));
      // Also save a fallback under 'lastResult' so result page can recover if specific ID is missing
      localStorage.setItem('lastResult', JSON.stringify(result));
      
      console.log("Result stored with ID:", resultId);
      console.log("Navigating to result page...");

      // Navigate with just the ID (much shorter URL)
        ...
        <div className="card mb-6">
          <div className="flex flex-wrap items-center justify-between gap-4">
            ...
            <div className="text-right">
              ...
              <p className={`text-2xl font-bold ${timeLeft < 60 ? 'text-red-600' : 'text-primary-600'}`}>
